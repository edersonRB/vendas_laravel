...
<body class="antialiased">
    ...
    <div class="min-h-screen">
        ...
        <div class="relative d-flex flex-column bg-white sm:items-center py-1 sm:pt-0">
            ...
            Venda {{$venda->id}} - data e hora: {{$venda->updated_at}}</br>
            <table class="table w-75 mt-0">
                ...
                <tbody id="corpo-tabela">
                    ...
                    @foreach($venda->produtos()->get() as $i => $produto)
                    <tr style="text-align: center;">
                        <th scope="row">{{$produto->id}}</th>
                        <td>{{$produto->marca}}</td>
                        <td>{{$produto->nome}}</td>
                        <td>R$ {{$produto->preco}}</td>
                        @foreach($listaProdutoVendas as $produto_venda)
                        @if($produto_venda->venda_id == $venda->id && $produto_venda->produto_id == $produto->id)
                        <td>{{$produto_venda->qtd}}</td>
                        <td>{{$produto->preco*$produto_venda->qtd}}</td>
                        @endif
                        @endforeach
