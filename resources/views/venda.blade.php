...
<body class="antialiased">
    ...
    <div class="min-h-screen">
            ...
            <div class="container-fluid">
                ...
                <div class="collapse navbar-collapse" id="navbarText">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                        <li class="nav-item">
                            <a class="nav-link active" aria-current="page" href="#">Nova Venda</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="./historico-vendas">Histórico</a>
                        </li>
                        <li class="nav-item">
        ...
        <div class="relative d-flex flex-column bg-white sm:items-center py-1 sm:pt-0">
            ...
            <h4 class="m-3"> Cadastre uma nova venda</h4>
            <form class="w-75 mb-3" method="post" action="{{route('adicionarItemVenda' , ['id_venda'=>$vendas[0]->id])}}">
                {{csrf_field()}}
                <div class="row">
                    <select class="col-8 form-control" name="produto" id="produto">
                        <option value="" disabled selected>Selecione o produto</option>
                        @foreach($listaProdutos as $produto)
                        <option value="{{$produto->id}}">{{$produto->marca}}-{{$produto->nome}} R${{$produto->preco}}</option>
                        @endforeach
                    </select>
                    <input class="col form-control" name="quantidade" id="quantidade" type="number" placeholder="Quantidade" required>
            ...
            <table class="table w-75 mt-0">
                ...
                <tbody id="corpo-tabela">
                    @if(count($vendas) > 0)
                    @foreach($produtosNaVenda as $produto)
                    ...
                    <tr style="text-align: center;">
                        ...
                        @foreach($listaQtds as $qtd)
                        @if($qtd->produto_id == $produto->id)
                        <td>x
                            {{$qtd->qtd}}
                        </td>
                        <td>R$
                            {{$qtd->qtd*$produto->preco}}
                        </td>

                        @endif
                        @endforeach
                        <td>
                            <form method="post" action="{{route('excluirItemVenda',['id_produto' => $produto->id, 'id_venda' =>$vendas[0]->id])}}">
                                {{csrf_field()}}
                                <button class="bg-transparent" type="submit">X</button>
                            </form>
                        </td>
                    ...
                    @else
                    <script>
                        window.location = "/adicionarVenda";
                    </script>
                    @endif
            ...
            <form method="post" action="{{route('finalizarVenda',['id_venda' => $vendas[0]->id])}}">
                {{csrf_field()}}
                @php $total = 0 @endphp
                @foreach($produtosNaVenda as $produto)
                @foreach($listaQtds as $qtd)
                @if($qtd->produto_id == $produto->id)
                @php $total+= $qtd->qtd*$produto->preco @endphp
                @endif
                @endforeach
                @endforeach
                @if(count($listaQtds)>0)
                <div class="container">
                    <div class="row">
                        <label class="col-4" for="total">TOTAL R$</label>
                        <input class="col" name="total" type="text" value="{{$total}}" readonly="readonly"></br>

                    </div>
                    <div class="row">
                        @if(count($listaQtds)>0)
                        <button class="col btn btn-success" type="submit">FINALIZAR VENDA</button>
                        @endif
                    </div>
                </div>
                @endif
            ...
            @else
            <h4 class="m-3"> Cadastre pelo menos um produto antes. <a href="/produtos">clique aqui para cadastrar</a></h4>
            @endif
